...
class GuardConfig:
    max_window_loss_pct: float
    pause_after_loss_steps: int
    pause_after_2_losses_steps: int
    max_trades_per_day: int
    fee_multiplier: float

    @property
    def key(self) -> str:
        loss = f"{self.max_window_loss_pct * 100:.0f}"
        fee = f"{self.fee_multiplier:g}".replace(".", "p")
        return (
            f"loss{loss}_pause{self.pause_after_loss_steps}_"
            f"pause2_{self.pause_after_2_losses_steps}_maxday{self.max_trades_per_day}_fee{fee}x"
        )
...
def _evaluate_window(
    ...
    loss_floor = initial_balance * (1.0 - guard.max_window_loss_pct)
    ...
    for step in range(start_step, end_limit):
        ...
        if position.side == 0:
            ...
            if entry_signal and balance <= loss_floor:
                ...
                flat_steps += 1
            elif entry_signal and trades_by_day[day] >= guard.max_trades_per_day:
                ...
                before = balance
                balance, position, fee = _open_position(balance, price, step, market, guard.fee_multiplier)
                if position.side > 0:
                    total_fees += fee
                    trades_by_day[day] += 1
            ...
            if close_reason and open_trade is not None:
                balance, trade = _close_trade(market, position, balance, step, open_trade, close_reason, guard.fee_multiplier)
                total_fees += float(trade["fees"]) - open_trade.entry_fee
                trades.append(trade)
                if float(trade["net"]) < 0.0:
                    consecutive_losses += 1
                    pause_until = max(pause_until, step + guard.pause_after_loss_steps)
                    if consecutive_losses >= 2:
                        pause_until = max(pause_until, step + guard.pause_after_2_losses_steps)
                else:
                    consecutive_losses = 0
                position = Position()
                open_trade = None
    ...
    if position.side != 0 and open_trade is not None:
        balance, trade = _close_trade(market, position, balance, end_limit, open_trade, "window_end", guard.fee_multiplier)
        total_fees += float(trade["fees"]) - open_trade.entry_fee
        trades.append(trade)
    ...
    return {
        ...
        "stopped_trade_count": int(guard_counts["max_window_loss"]),
...
def _summary(windows: list[dict[str, Any]], initial_balance: float) -> dict[str, Any]:
    ...
    return {
        ...
        "stopped_trade_count": int(sum(w["stopped_trade_count"] for w in windows)),
...
def run_guard(
    ...
    guards = [
        GuardConfig(loss, pause1, pause2, max_day, fee)
        for fee, loss, pause1, pause2, max_day in product(
            FEE_MULTIPLIERS,
            MAX_WINDOW_LOSS_PCTS,
            PAUSE_AFTER_LOSS_STEPS,
            PAUSE_AFTER_2_LOSSES_STEPS,
            MAX_TRADES_PER_DAY,
        )
    ]
        ...
        config_reports.append(
            {
                "config_id": guard.key,
                "config": {
                    "fee_multiplier": guard.fee_multiplier,
                    "max_window_loss_pct": guard.max_window_loss_pct,
                    "pause_after_loss_steps": guard.pause_after_loss_steps,
                    "pause_after_2_losses_steps": guard.pause_after_2_losses_steps,
    ...
    output_path = output_dir / f"long_edge_risk_guard_{created_at}.json"
