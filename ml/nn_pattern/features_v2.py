...
def build_feature_frame(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    ...
    for i, f in enumerate(frames):
        print(f"DEBUG: Frame {i} shape: {f.shape}")

    feature_frame = pd.concat(frames, axis=1)
    print(f"DEBUG: build_feature_frame columns: {len(feature_frame.columns)}")
    print(f"DEBUG: Columns: {feature_frame.columns.tolist()}")
