"""Fail-closed promotion gate for V8 research evidence."""
...
def evaluate_v8_gate(validation: Mapping[str, Any]) -> Mapping[str, Any]:
    blockers = []
    ...
    for side in ("LONG", "SHORT"):
        ...
        if not bool(result.get("validation_pass")):
            blockers.append(f"{side}_VALIDATION_FAILED")
        if int(result.get("passing_folds", 0)) < 3:
            blockers.append(f"{side}_POSITIVE_FOLDS_INSUFFICIENT")
        if int(result.get("router_skilled_folds", 0)) < 3:
            blockers.append(f"{side}_ROUTER_SKILL_INSUFFICIENT")
        if int(result.get("late_detector_skilled_folds", 0)) < 3:
            blockers.append(f"{side}_LATE_DETECTOR_SKILL_INSUFFICIENT")
        if not bool(result.get("worst_fold_non_negative")):
            blockers.append(f"{side}_WORST_FOLD_NEGATIVE")
        if not bool(result.get("leave_one_symbol_out", {}).get("passed")):
