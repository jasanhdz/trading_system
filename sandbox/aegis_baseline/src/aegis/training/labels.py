"""Versioned path-aware directional labels over finalized future candles."""
...
LONG_LABEL_SCHEMA_VERSION = "aegis-labels-long-v1"
...
class LongLabelConfig:
    ...
    def __post_init__(self) -> None:
        if self.entry_rule not in {"SIGNAL_CLOSE", "NEXT_BAR_OPEN"}:
            raise ValueError("unsupported LONG label entry rule")
...
class LongPathLabel:
    ...
    stopped_before_hit: bool = False
    ambiguous_hit_stop: bool = False
    terminal_long_return: float | None = None
    round_trip_cost_fraction: float | None = None
...
def _quarantine_long(config: LongLabelConfig, reason: str) -> LongPathLabel:
    return LongPathLabel(
...
def build_long_path_label(
    signal: Candle,
    future: Sequence[Candle],
    config: LongLabelConfig | None = None,
) -> LongPathLabel:
    """Label one LONG hypothesis using only closed bars t+1 through t+H."""
    settings = config or LongLabelConfig()
    if len(future) != settings.horizon_bars:
        return _quarantine_long(settings, "INCOMPLETE_HORIZON")
    interval = timedelta(minutes=5)
    previous_close = signal.close_time
    for candle in future:
        if not candle.is_closed:
            return _quarantine_long(settings, "NON_FINAL_CANDLE")
        if (
            candle.open_time != previous_close
            or candle.close_time - candle.open_time != interval
        ):
            return _quarantine_long(settings, "FUTURE_GAP")
        previous_close = candle.close_time
    entry = (
        future[0].open
        if settings.entry_rule == "NEXT_BAR_OPEN"
        else signal.close
    )
    favorable = [
        max(0.0, (candle.high - entry) / entry) for candle in future
    ]
    adverse = [
        max(0.0, (entry - candle.low) / entry) for candle in future
    ]
    mfe = max(favorable)
    mae = max(adverse)
    mfe_index = favorable.index(mfe) + 1 if mfe > 0.0 else None
    mae_index = adverse.index(mae) + 1 if mae > 0.0 else None
    mfe_before_mae = mfe_index is not None and (
        mae_index is None or mfe_index < mae_index
    )
    ratio = mfe / mae if mae > 0.0 else math.inf if mfe > 0.0 else 0.0
    initial_adverse = max(adverse[: min(3, len(adverse))], default=0.0)
    net_quality = mfe - mae - settings.round_trip_cost_fraction
    hit, stopped, ambiguous = _long_hit_before_stop(
        entry,
        future,
        target_fraction=settings.clean_mfe_fraction,
        stop_fraction=settings.bad_mae_fraction,
    )
    clean = (
        not ambiguous
        and mfe_before_mae
        and mfe >= settings.clean_mfe_fraction
        and mae <= settings.clean_mae_fraction
        and ratio >= settings.clean_mfe_mae_ratio
        and mfe_index is not None
        and mfe_index <= settings.horizon_bars * settings.clean_time_fraction
        and net_quality > 0.0
        and initial_adverse <= settings.initial_adverse_3_fraction
    )
    mae_before_mfe = mae_index is not None and (
        mfe_index is None or mae_index <= mfe_index
    )
    bad = (
        (mae_before_mfe and mae >= settings.bad_mae_fraction)
        or (
            mfe < settings.bad_low_mfe_fraction
            and mae >= 0.04 / settings.historical_reference_leverage
        )
        or initial_adverse >= settings.bad_initial_adverse_3_fraction
        or net_quality < 0.0
        or (
    ...
    return LongPathLabel(
        schema_version=LONG_LABEL_SCHEMA_VERSION,
        valid=True,
        quarantine_reason=None,
        entry_convention=settings.entry_rule,
        ...
        mfe_mae_ratio=ratio,
        time_to_mfe=mfe_index,
        time_to_mae=mae_index,
        mfe_before_mae=mfe_before_mae,
        clean_entry=clean,
        bad_entry=bad,
        tail_event=mae >= settings.tail_mae_fraction,
        hit_before_stop=hit,
        stopped_before_hit=stopped,
        ambiguous_hit_stop=ambiguous,
        terminal_long_return=(future[-1].close - entry) / entry,
        round_trip_cost_fraction=settings.round_trip_cost_fraction,
    )
