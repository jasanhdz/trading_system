...
def test_short_scenario_freezes_and_records(snapshot_factory, scenario_runtime_factory) -> None:
    side = "SHORT"
...
def test_long_scenario_is_not_promotable(snapshot_factory, scenario_runtime_factory) -> None:
    snapshot = snapshot_factory()
    runtime = scenario_runtime_factory("LONG", snapshot)
    request = DecisionRequest("request-LONG", "cycle-LONG", "aegis-decision-request-v1",
                              runtime.config.contract_version, runtime.config.config_version, snapshot)
    response = runtime.evaluate(request)
