...
def main() -> int:
    ...
    parser.add_argument(
        "--mode", choices=("stage-0", "ablations", "stage-1b", "stage-4b"),
        default="stage-0",
    )
    parser.add_argument(
    ...
    try:
        ...
        elif args.mode == "stage-1b":
            result = run_stage_1b(config)
            verdict = result["answer"]
        else:
            result = run_stage_4b(config)
            verdict = "STAGE_4B_ATTRIBUTION_COMPLETE"
        ...
        return 5
    except FollowupNondeterministic as error:
        print(json.dumps({"verdict": "NONDETERMINISTIC", "error": str(error)}))
        return 6
    except FollowupProtocolError as error:
        print(json.dumps({"verdict": "PROTOCOL_VIOLATION", "error": str(error)}))
        return 7
