...
def download_and_save(symbol, timeframe, days=1000):
    ...
    while since < exchange.milliseconds():
        try:
            ...
            # Save intermediate to avoid memory issues
...
def _save_batch(symbol, timeframe, ohlcv_list):
    ...
    # The DB seems to use "BTC/USDT" format based on previous check
    ...
    try:
        # Check if insert_ohlcv_data exists, otherwise implement manual insert
...
if __name__ == "__main__":
    # Download 1h and 4h
    download_and_save("BTCUSDT", "1h", 1000)
    download_and_save("BTCUSDT", "4h", 1000)
