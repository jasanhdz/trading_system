...
def run_grid_search(symbol: str, days: int = 3, hours: int = 0):
    """
    Prueba diferentes configuraciones de Threshold y Hard Stop para encontrar el óptimo.
    """
    ...
    print(f"   Configuraciones a probar: {len(base_thresholds) * len(hard_stop_options)}")
    print(f"{'='*70}\n")
    ...
    total_combos = len(base_thresholds) * len(hard_stop_options)
    combo_num = 0
    ...
    for base_thr in base_thresholds:
        for stop_pct in hard_stop_options:
            ...
            print(f"[{combo_num}/{total_combos}] Thr:{base_thr:.2f} | Stop:{stop_pct:.0%} -> Return: {return_pct:>+6.2f}% | Trades: {num_trades} | WinRate: {win_rate:.0%}")
    ...
    # 7. Imprimir Mejor Configuración
