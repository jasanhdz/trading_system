...
print("🚀 Iniciando Motor de Backtest Dual (V30 Entradas + V3 Salidas)")
...
import torch
print("\n🧠 Cargando IA de Entradas (V31 Champion)...")
entry_model = PPO.load(ENTRY_MODEL_PATH, device="cpu")
print("🧠 Cargando IA de Salidas (V3 Exit Champion)...")
...
for i in range(WINDOW, len(df_play) - 1):
    ...
    if in_trade:
        ...
        if close_reason is None:
            time_decay = (trade_step * 5.0) / 120.0
            atr_norm = (df_play['atr'].iloc[i] / entry_price) * LEVERAGE
            dd_from_peak = (mfe_roe - current_roe) / max(mfe_roe, 1e-10) if mfe_roe > 0 else 0.0
            
            roe_hist = []
            for j in range(max(0, i-2), i+1):
                p = df_play['close'].iloc[j]
                r = ((p - entry_price) / entry_price) * side * LEVERAGE
                roe_hist.append(r)
                
            if len(roe_hist) == 3:
                roe_vel = np.clip(roe_hist[2] - roe_hist[1], -2.0, 2.0)
                roe_acc = np.clip((roe_hist[2] - roe_hist[1]) - (roe_hist[1] - roe_hist[0]), -2.0, 2.0)
            else:
                roe_vel = 0.0; roe_acc = 0.0
            
            dist_tp = max(0.0, (1.50 - current_roe) / 1.50)
            ...
            obs_exit = np.array([
                ...
                time_decay,
                atr_norm,
                np.clip(dd_from_peak, 0.0, 5.0),
                roe_vel,
                roe_acc,
                df_play['cvd_z'].iloc[i],
                df_play['cvd_roc'].iloc[i],
                np.clip(dist_tp, 0.0, 5.0),
                df_play['vol_ratio'].iloc[i]
            ], dtype=np.float32)
            
            action_exit, _ = exit_model.predict(obs_exit, deterministic=True)
            action_exit = int(action_exit)
            
            if action_exit == 1: # AI quiere cerrar
                try:
                    obs_t = exit_model.policy.obs_to_tensor(obs_exit)[0]
                    dist = exit_model.policy.get_distribution(obs_t)
...
# === REPORT ===
print(f"\n{'='*50}")
print(f"🏆 BACKTEST DUAL COMPLETADO (Últimos 20 días)")
print(f"{'='*50}")
